...
# ========= 路徑設定（請改成你的 Windows 路徑） =========
# 例：
# MODEL_PATH = r"C:\Users\you\Desktop\best.pt"
# VIDEO_PATH = r"C:\Users\you\Videos\demo.mp4"
...
VIDEO_PATH = r"D:\dav2mp4\2025-10-08\001\100812.mp4"
...
if not cap.isOpened():
    p("? 無法開啟影片，請確認編解碼器可用（建議安裝 ffmpeg 或使用 mp4v）"); sys.exit(1)
...
# ========= 追蹤器設定（使用 BoT-SORT，Ultralytics 內建支援，含 ReID） =========
TRACKER_CFG = str(Path(ul.__file__).parent / "cfg" / "trackers" / "botsort.yaml")

# ========= 統計用結構（用追蹤器給的 pid） =========
K_PERSON = 20   # 人員累積 K 幀視為一位
M_TAKE   = 10   # 拿書累積 M 幀視為確立一次
DEBUG_PRINT = True
...
def init_video_writer(size):
    ...
    if vw is not None:
        ...
        return
    raise RuntimeError("VideoWriter 初始化失敗：請確認系統編碼器/權限。")

# ========= 主迴圈（YOLO .track + BoT-SORT） =========
p("開始處理（YOLO tracking + BoT-SORT）…")
frame_idx = 0
